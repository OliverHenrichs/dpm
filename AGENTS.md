...
One known gap, deliberately out of scope so far:
...
- `expo-localization` is listed in `app.config.ts` → `plugins`, not left to autolinking, for the reason the comment there gives: config plugins are not applied by autolinking. It is only read on the startup path, through `src/settings/data/DeviceLocale.ts`. Adding it left the `npm audit` baseline unchanged at three moderate findings.
